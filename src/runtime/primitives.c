...
static Val VMType(u32 num_args, VM *vm);
static Val VMHead(u32 num_args, VM *vm);
static Val VMTail(u32 num_args, VM *vm);
...
{
  Val arg;
  if (num_args == 0) return Error;
  arg = StackPop(vm);
  vm->stack.count -= num_args-1;

  if (IsFloat(arg) || IsInt(arg)) {
    return Sym("number", &vm->symbols);
  } else if (IsSym(arg)) {
    return Sym("symbol", &vm->symbols);
  } else if (IsPair(arg)) {
    return Sym("pair", &vm->symbols);
  } else if (IsTuple(arg, &vm->mem)) {
    return Sym("tuple", &vm->symbols);
  } else if (IsBinary(arg, &vm->mem)) {
    return Sym("binary", &vm->symbols);
  } else {
    return Error;
  }
}
...
static Val VMHead(u32 num_args, VM *vm)
{
  ...
  arg = StackPop(vm);
  if (!IsPair(arg)) return Error;
  return Head(arg, &vm->mem);
}
