...
{
  ASTNode *node = MakeNode(errorNode, p);
  Error *error =
      NewError(NewString("Parse error: ^"), 0, p->token.pos, p->token.length);
...
static ASTNode *ParseText(Parser *p);
...
ASTNode *ParseModule(char *source)
{
  ...
  if (!Ok(result)) return ParseFail(node, result);
...
static ASTNode *ParseImportList(Parser *p)
{
  ...
  do {
    ...
    if (!Ok(import)) return ParseFail(node, import);
...
static ASTNode *ParseStmts(Parser *p)
{
  ...
  do {
    VSpacing(p);
    stmt = ParseStmt(p);
    if (!Ok(stmt)) return ParseFail(node, stmt);
    NodePush(node, stmt);
...
static ASTNode *ParseOp(ASTNode *expr, Parser *p)
{
  ...
  if (!Ok(arg)) return ParseFail(expr, arg);
...
static ASTNode *ParseCall(ASTNode *expr, Parser *p)
{
  ...
  if (!MatchToken(rparenToken, p)) {
    args = ParseItems(p);
    if (!Ok(args)) return ParseFail(node, args);
    NodePush(node, args);
    if (!MatchToken(rparenToken, p)) return Expected(")", node, p);
  } else {
...
static ASTNode *ParseAccess(ASTNode *expr, Parser *p)
{
  ...
  arg = ParseExpr(p);
  if (!Ok(arg)) return ParseFail(node, arg);
  NodePush(arg, node);
...
static ASTNode *ParseLambda(Parser *p)
{
  ...
  if (!Ok(params)) return ParseFail(node, params);
  NodePush(node, params);
  if (!MatchToken(arrowToken, p)) return Expected("->", node, p);
  VSpacing(p);
  body = ParseExpr(p);
  if (!Ok(body)) return ParseFail(node, body);
  NodePush(node, body);
  return node;
}
...
static ASTNode *ParseLet(Parser *p)
{
  ...
  if (!Ok(assigns)) return ParseFail(node, assigns);
  NodePush(node, assigns);
  VSpacing(p);
  if (!MatchToken(inToken, p)) return Expected("in", node, p);
  VSpacing(p);
  expr = ParseExpr(p);
  if (!Ok(expr)) return ParseFail(node, expr);
  NodePush(node, expr);
  node->end = p->token.pos;
  return node;
}
...
static ASTNode *ParseAssigns(Parser *p)
{
  ...
  do {
    ...
    if (!Ok(assign)) return ParseFail(node, assign);
...
static ASTNode *ParseListTail(Parser *p)
{
  ...
  if (MatchToken(rbracketToken, p)) return NilNode(p);
  ...
  if (MatchToken(commaToken, p)) {
    ...
    if (!Ok(tail)) return ParseFail(item, tail);
  } else {
    tail = NilNode(p);
  }
...
{
  ASTNode *node;
  if (CheckToken(nilToken, p)) {
    node = NilNode(p);
  } else if (CheckToken(trueToken, p)) {
    node = MakeTerminal(constNode, IntVal(1), p);
  } else if (CheckToken(falseToken, p)) {
...
static ASTNode *ParseItems(Parser *p)
{
  ...
  if (!Ok(item)) return ParseFail(node, item);
  NodePush(node, item);
  VSpacing(p);
  while (MatchToken(commaToken, p)) {
    VSpacing(p);
...
static ASTNode *ParseIDList(Parser *p)
{
  ...
  node = MakeNode(listNode, p);
  id = ParseID(p);
  if (!Ok(id)) return ParseFail(node, id);
  NodePush(node, id);
  Spacing(p);
  while (MatchToken(commaToken, p)) {
    VSpacing(p);
...
static ASTNode *ParseID(Parser *p)
{
  ...
  node = MakeTerminal(varNode, sym, p);
  Adv(p);
  return node;
}
